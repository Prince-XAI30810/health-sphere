...
export const getPastPrescriptions = (): Prescription[] => {
    return prescriptions.filter(p => !p.isActive);
};

// Get active prescription count
export const getActivePrescriptionCount = (): number => {
    return prescriptions.filter(p => p.isActive).length;
};

// Generate PDF Prescription
export const generatePrescriptionPDF = (prescription: Prescription): string => {
    const doc = new jsPDF();

    const primaryColor = [20, 120, 120]; // Teal color to match theme
...
export const downloadPrescriptionPDF = (prescription: Prescription): void => {
    const pdfDataUrl = generatePrescriptionPDF(prescription);
